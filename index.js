...
bot.action(/^approve_(\d+)_(.+)$/, async (ctx) => {
    ...
    try {
        ...
        ctx.answerCbQuery("❌ Error aaya, dobara try karo");
    }
});
...
bot.command("mystatus", async (ctx) => {
    ...
    try {
        ...
        if (!user.payments || user.payments.length === 0) {
            return ctx.reply("📭 Koi payment history nahi hai.\n/buy se premium lo.");
        }
...
bot.action("admin_pending", async (ctx) => {
    ...
    try {
        ...
        await ctx.editMessageText(
            `⏳ Pending Payments (${usersWithPending.length} users)\n\nHar payment ka detail alag message mein aa raha hai:`,
            Markup.inlineKeyboard([[Markup.button.callback("🔙 Back", "admin_back")]])
        );
        for (const u of usersWithPending) {
            ...
            for (const p of pending) {
                ...
                if (p.screenshotFileId) {
                    await ctx.telegram.sendPhoto(ADMIN_ID, p.screenshotFileId, {
                        caption: msgText,
                        reply_markup: pendingPaymentKeyboard(u.telegramId, p.plan).reply_markup,
                    });
                } else {
                    await ctx.telegram.sendMessage(ADMIN_ID, msgText, {
                        reply_markup: pendingPaymentKeyboard(u.telegramId, p.plan).reply_markup,
                    });
                }
                await sleep(300);
        ...
        console.error("admin_pending:", err.message);
        ctx.reply("❌ Error");
    }
});
...
bot.action("admin_coins_info", (ctx) => {
    if (Number(ctx.from.id) !== ADMIN_ID) return ctx.answerCbQuery("❌");
    ctx.answerCbQuery();
    ctx.reply(
        `🪙 Coins Commands:\n\n` +
...
bot.action("admin_prem_info", (ctx) => {
    if (Number(ctx.from.id) !== ADMIN_ID) return ctx.answerCbQuery("❌");
    ctx.answerCbQuery();
    ctx.reply(
        `👑 Premium Commands:\n\n` +
...
bot.action("admin_search_info", (ctx) => {
    if (Number(ctx.from.id) !== ADMIN_ID) return ctx.answerCbQuery("❌");
    ctx.answerCbQuery();
    ctx.reply("🔍 User search:\n/userinfo USER_ID\n\nUsername se:\n/finduser @username");
});

bot.action("admin_allcmds", (ctx) => {
    if (Number(ctx.from.id) !== ADMIN_ID) return ctx.answerCbQuery("❌");
    ctx.answerCbQuery();
    ctx.reply(
        `📋 ALL ADMIN COMMANDS\n\n` +
...
bot.command("removepremium", async (ctx) => {
    ...
    try {
        ...
        try { await bot.telegram.sendMessage(userId, "⚠️ Aapka premium admin ne remove kar diya hai."); } catch (e) {}
    } catch (err) {
        ctx.reply("❌ Error");
    }
});
...
bot.command("unbanuser", async (ctx) => {
    ...
    try {
        ...
        try { await bot.telegram.sendMessage(userId, "✅ Aapka ban hat gaya hai! /start karein."); } catch (e) {}
    } catch (err) {
        ctx.reply("❌ Error");
    }
});
...
bot.command("userinfo", async (ctx) => {
    ...
    try {
        ...
        ctx.reply(
            ...
            `Premium: ${user.premium ? `✅ ${user.premiumPlan} (${user.premiumExpires?.toLocaleDateString("en-IN")})` : "❌"}\n` +
            `Banned: ${user.banned ? `🚫 Yes (${user.banReason})` : "✅ No"}\n` +
            `Payments: ${approvedCount} approved, ${pendingCount} pending\n` +
...
bot.command("finduser", async (ctx) => {
    ...
    try {
        ...
        ctx.reply(
            ...
            `💳 Payments: ${approvedCount} approved, ${pendingCount} pending\n` +
            `📅 Joined: ${user.joinDate?.toLocaleDateString("en-IN")}`
        );
    } catch (err) {
        ctx.reply("❌ Error");
    }
...
bot.command("broadcast", async (ctx) => {
    ...
    try {
        ...
        for (const user of users) {
            try {
                await bot.telegram.sendMessage(user.telegramId, `📢 GrowMate AI\n\n${message}`);
                success++;
            } catch (e) { failed++; }
            await sleep(100);
        }
...
bot.command("broadcastpremium", async (ctx) => {
    ...
    try {
        ...
        if (!message) return ctx.reply("Use:\n/broadcastpremium Hello premium users!");
        const users = await User.find({ banned: false, premium: true });
        let success = 0, failed = 0;
        await ctx.reply(`📢 Premium broadcast shuru... (${users.length} premium users)`);
        for (const user of users) {
            try {
                await bot.telegram.sendMessage(user.telegramId, `👑 Premium Member Update\n\n${message}`);
                success++;
            } catch (e) { failed++; }
            await sleep(100);
        }
        ctx.reply(`✅ Premium Broadcast Done!\n✅ Sent: ${success}\n❌ Failed: ${failed}`);
    } catch (err) {
        ctx.reply("❌ Error");
    }
});
...
bot.command("broadcastnew", async (ctx) => {
    ...
    try {
        ...
        if (!message) return ctx.reply("Use:\n/broadcastnew Hello new users!");
        const sevenDaysAgo = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000);
        const users = await User.find({ banned: false, joinDate: { $gte: sevenDaysAgo } });
        let success = 0, failed = 0;
        await ctx.reply(`📢 New users broadcast shuru... (${users.length} new users)`);
        for (const user of users) {
            try {
                await bot.telegram.sendMessage(user.telegramId, `🆕 Welcome New Member!\n\n${message}`);
                success++;
            } catch (e) { failed++; }
            await sleep(100);
        }
        ctx.reply(`✅ New Users Broadcast Done!\n✅ Sent: ${success}\n❌ Failed: ${failed}`);
    } catch (err) {
        ctx.reply("❌ Error");
    }
});
...
// ═══════════════════════════════════════════════
//  CRON — Premium Expiry Check Every 6 Hours
// ═══════════════════════════════════════════════
